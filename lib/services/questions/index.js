...
import { getNuclearQuestionPrompt } from '@/lib/llm/prompts/nuclear';
  ...
  try {
    ...
    if (useGaExpansion) {
      ...
      for (const gaPair of activeGaPairs) {
        ...
        let prompt;
        // 检查是否开启核应急模式
        if (model && model.generationMode === 'nuclear') {
             prompt = await getNuclearQuestionPrompt(
              language,
              {
                text: chunk.content,
                number: baseQuestionNumber,
                type: 'EOP' // 默认为规程问答，后续可扩展
              },
              projectId
            );
        } else {
             prompt = await getQuestionPrompt(
              language,
              {
                text: chunk.content,
                number: baseQuestionNumber,
